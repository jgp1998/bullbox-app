...
export async function login(page: Page) {
  ...
  if (!(await usernameField.isVisible())) {
    try {
      ...
      await Promise.race([
        page.waitForSelector('[data-testid="login-username"]', {
          timeout: 15000,
        }),
      ...
      throw new Error(
        "Neither login screen nor logout button appeared after 15s",
      );
    }
  }
  ...
  if (!email || !password) {
    throw new Error(
      "Test user credentials not found in env (TEST_USER_EMAIL or TEST_USER_PASSWORD)",
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Login timed out or failed. Could not find logout-button. Original error: ${e.message}`,
    );
  }
