...
vi.mock('./PlateBreakdown', () => ({
    default: ({ totalWeight }: any) => <div data-testid="plate-breakdown">Breakdown for {totalWeight.toFixed(1)}</div>
}));
...
describe('PercentageCalculator', () => {
    ...
    it('renders and selects the first exercise by default', () => {
        ...
        const select = screen.getByTestId('select-percentageCalculator.exerciseLabel') as HTMLSelectElement;
    ...
    it('calculates the correct percentage of 1RM', () => {
        ...
        const percInput = screen.getByTestId('input-percentageCalculator.percentageLabel');
        fireEvent.change(percInput, { target: { value: '80' } });
        
        const calcButton = screen.getByRole('button', { name: 'percentageCalculator.calculate' });
        ...
        expect(screen.getByTestId('plate-breakdown')).toHaveTextContent('Breakdown for 93.3');
    });
    ...
    it('updates results when calculation is rerun with different percentage', () => {
        ...
        fireEvent.click(screen.getByRole('button', { name: 'percentageCalculator.calculate' }));
    ...
    it('shows message when no PBs are available', () => {
        ...
        expect(screen.getByRole('button', { name: 'percentageCalculator.calculate' })).toBeDisabled();
    });
});
