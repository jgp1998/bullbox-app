        ...
        <div className="space-y-8">
            <section className="animate-in">
                <div className="flex items-center space-x-3 mb-3">
                    ...
                    <h3 className="text-lg sm:text-xl font-bold text-(--accent) line-clamp-1">{t('modals.analysis')}</h3>
                </div>
                <div className="bg-white/5 rounded-xl p-4 sm:p-5">
                    <p className="text-sm sm:text-base leading-relaxed text-(--text) whitespace-pre-line">{result.analysis}</p>
                </div>
            </section>
            ...
            <section className="animate-in" style={{ animationDelay: '100ms' }}>
                <div className="flex items-center space-x-3 mb-3">
                    ...
                    <h3 className="text-lg sm:text-xl font-bold text-(--primary) line-clamp-1">{t('modals.trainingTips')}</h3>
                </div>
                <div className="space-y-3">
                    {result.trainingTips.map((tip, index) => (
                        <div key={index} className="flex items-start space-x-3 text-sm sm:text-base text-(--text) bg-white/5 p-3 rounded-lg border border-white/5">
                            <span className="shrink-0 w-6 h-6 rounded-full bg-(--primary)/20 text-(--primary) flex items-center justify-center text-xs font-bold mt-0.5">
                                {index + 1}
                            </span>
            ...
            <section className="animate-in" style={{ animationDelay: '200ms' }}>
                <div className="flex items-center space-x-3 mb-3">
                    ...
                    <h3 className="text-lg sm:text-xl font-bold text-(--accent) line-clamp-1">{t('modals.nutritionSuggestion')}</h3>
                </div>
                <div className="bg-white/5 rounded-xl p-4 sm:p-5 border-l-4 border-(--accent)">
                    <p className="text-sm sm:text-base leading-relaxed text-(--text)">{result.nutritionSuggestion}</p>
                </div>
            </section>
        </div>
    );
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-0 sm:p-4 transition-opacity duration-300">
      ...
      >
        ...
        <div className="p-6 sm:p-8 overflow-y-auto">
          <button
            onClick={onClose}
            className="absolute top-4 right-4 sm:top-6 sm:right-6 text-(--muted-text) hover:text-(--primary) transition-colors p-2 rounded-full hover:bg-white/5"
            aria-label="Close"
          >
            <XIcon className="w-6 h-6" />
          </button>
          
          <h2 className="text-xl sm:text-2xl font-bold mb-6 pr-8 text-(--primary)">{t('modals.aiPerformanceReview')}</h2>
