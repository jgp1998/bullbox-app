...
import { useHistoryAnalysisStore } from './useHistoryAnalysisStore';
...
describe('useHistoryAnalysisStore', () => {
    ...
    it('should set error state on getAnalysis failure', async () => {
        ...
        vi.mocked(historyService.getTrainingAdvice).mockRejectedValueOnce(new Error(errorMessage));

        const state = useHistoryAnalysisStore.getState();
        const dummyRecord = { id: '1', date: '2023-01-01', exercise: 'Squat', value: 100, type: 'Weight', reps: 5, weight: 100 } as HistoryRecord;
    ...
    it('should set analysis result on getAnalysis success', async () => {
        const mockResult = { analysis: 'Good', trainingTips: ['Tip 1'], nutritionSuggestion: 'Eat' };
        vi.mocked(historyService.getTrainingAdvice).mockResolvedValueOnce(mockResult);
    ...
    it('should set exercise detail on getExerciseDetails success', async () => {
        const mockDetail = { description: 'A test', bestPractices: ['Do this'], commonMistakes: ['Not this'] };
        vi.mocked(historyService.getExerciseDetails).mockResolvedValueOnce(mockDetail);
