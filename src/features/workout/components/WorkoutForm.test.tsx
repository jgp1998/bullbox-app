...
vi.mock('@/components/ui/Button', () => ({
    default: ({ onClick, children, type, title, 'aria-label': ariaLabel }: any) => (
        <button 
...
vi.mock('@/components/ui/Input', () => ({
    default: ({ label, value, onChange, type, options, id, ...props }: any) => (
        <div>
            ...
            {type === 'select' ? (
                <select id={id} data-testid={`input-${id}`} value={value} onChange={onChange} {...props}>
                    {options?.map((opt: any) => (
...
describe('WorkoutForm', () => {
    ...
    it('submits the form with weight and reps', () => {
        ...
        fireEvent.change(screen.getByTestId('input-exercise'), { target: { value: 'Squat' } });
        ...
        expect(mockOnAddRecord).toHaveBeenCalledWith(expect.objectContaining({
            exercise: 'Squat',
            weight: 100,
            reps: 5,
            unit: 'kg'
    ...
    it('submits the form with time instead of weight', () => {
        ...
        fireEvent.change(screen.getByTestId('input-exercise'), { target: { value: 'Bench Press' } });
    ...
    it('shows error if no metrics are provided', () => {
        ...
        expect(screen.getByText('workoutForm.errors.atLeastOneMetric')).toBeInTheDocument();
