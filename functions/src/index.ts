...
// Response Schames for Gemini (structured output) using inline objects to avoid import issues
...
export const getTrainingAdvice = onCall(
  ...
  async (request) => {
    ...
    if (!record || !history) {
      throw new HttpsError(
        "invalid-argument",
        "The function must be called with 'record' and 'history'.",
      );
    }
  ...
  `;
    ...
    try {
      ...
      // If it's a 404, it might be the model name or API not ready
      if (error.status === 404) {
        throw new HttpsError(
          "internal",
          `Gemini Model not found. Please ensure Generative Language API is enabled and the model 'gemini-1.5-flash' is available for your API key.`,
        );
      }

      throw new HttpsError(
        "internal",
        "Failed to generate advice from Gemini API.",
      );
    }
  },
);
...
export const getExerciseDetails = onCall(
  ...
  async (request) => {
    ...
    if (!exerciseName) {
      throw new HttpsError(
        "invalid-argument",
        "The function must be called with 'exerciseName'.",
      );
    }
  ...
  `;
    ...
    try {
      ...
      throw new HttpsError(
        "internal",
        "Failed to get exercise details from Gemini API.",
      );
    }
  },
);
